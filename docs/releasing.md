...
This procedure prevents source, citation, GitHub, and PyPI versions from diverging. Every version-facing file and artifact for this release must identify openWFN 0.7.2.
...
2. Confirm that `[project].version` in `pyproject.toml` is `0.7.2`.
...
6. Install the wheel into a newly created temporary virtual environment:
   ...
   "$release_smoke/venv/bin/python" -m pip install dist/openwfn-0.7.2-py3-none-any.whl
   ```

7. Verify the installed version, console entry point, and reference workflow:

   ```bash
   "$release_smoke/venv/bin/python" -c "import openwfn; assert openwfn.__version__ == '0.7.2'"
...
8. Inspect the wheel and confirm that it contains all Python modules, `assets/3Dmol-min.js`, package metadata, the license, and the console entry point:
   ...
   ```bash
   unzip -l dist/openwfn-0.7.2-py3-none-any.whl
   ```
...
10. Create the annotated release tag:
    ...
    ```bash
    git tag -a v0.7.2 -m "openWFN 0.7.2"
    ```
...
13. Verify PyPI from another clean environment:
    ...
    ```bash
    python -m pip install --no-cache-dir openwfn==0.7.2
    python -c "import openwfn; assert openwfn.__version__ == '0.7.2'"
    ```
